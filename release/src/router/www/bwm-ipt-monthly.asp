...
<content>
			...
			if (monthly_history.length > 0) {
				...
				for (i = 0; i < monthly_history.length; ++i) {
					...
					if ((b[1] == getNetworkAddress(nvram.lan_ipaddr,nvram.lan_netmask)) ||
						...
						(b[1] == getNetworkAddress(nvram.lan4_ipaddr,nvram.lan4_netmask)) ||
						(b[1] == getNetworkAddress(nvram.lan5_ipaddr,nvram.lan5_netmask)) ||
						(b[1] == getNetworkAddress(nvram.lan6_ipaddr,nvram.lan6_netmask)) ||
						(b[1] == getNetworkAddress(nvram.lan7_ipaddr,nvram.lan7_netmask))) {
	...
	<script type="text/javascript">
		var c;
		c = [];
		c.push({ title: 'List only these IPs', name: 'f_filter_ip', size: 50, maxlen: 255, type: 'text', help: 'Comma separated list' });
		...
		c.push({ title: 'Show subnet totals', name: 'f_subnet', type: 'checkbox', tip: 'Not considered when calculating total traffic on the last line' });
